...
for i, b in enumerate(b_vals):
    for j, l in enumerate(l_vals):
        ...
        try:
            try:
                result = pd.read_csv(f'outputfiles/gaia_catalog_data/stars_l{l:.3f}_b{b:.3f}.csv')
                print('gaia file loaded', l, b)
            except:
                # GAIA
                ...
                #pdb.set_trace()
                res_pd = result.to_pandas()
                res_pd.to_csv(f'outputfiles/gaia_catalog_data/stars_l{l:.3f}_b{b:.3f}.csv')
            ...
            if len(pm_l_cosb) > 0:
                stats_entry['gaia_l_median'] = np.median(pm_l_cosb)
                stats_entry['gaia_l_std'] = np.std(pm_l_cosb)
                stats_entry['gaia_b_median'] = np.median(pm_b)
                stats_entry['gaia_b_std'] = np.std(pm_b)
            else:
                stats_entry['gaia_l_median'] = np.nan
                stats_entry['gaia_l_std'] = np.nan
                stats_entry['gaia_b_median'] = np.nan
                stats_entry['gaia_b_std'] = np.nan

            # SYNTHPOP
            try:
                cat2 = pd.read_csv(f'outputfiles/default/gaia_pm_l{l:.3f}_b{b:.3f}.csv')
                print('synthpop file loaded', l,b)
            except:
            ...
            if len(mul_vals) > 0:
                stats_entry['synth_l_median'] = np.median(mul_vals)
                stats_entry['synth_l_std'] = np.std(mul_vals)
                stats_entry['synth_b_median'] = np.median(mub_vals)
                stats_entry['synth_b_std'] = np.std(mub_vals)
            else:
                stats_entry['synth_l_median'] = np.nan
                stats_entry['synth_l_std'] = np.nan
                stats_entry['synth_b_median'] = np.nan
                stats_entry['synth_b_std'] = np.nan
...
stats_df.to_csv("gaia_vs_synthpop_new_run.csv", index=False)
