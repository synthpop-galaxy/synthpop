...
synthpop_nonmag_bin_cols = ['system_idx', 'zams_mass', 'Teff', 'L',
...
class PopsyclePostProcessing(PostProcessing):
    ...
    def do_post_processing(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        dataframe.drop(columns=['phase'], inplace=True)
        ...
        if "Is_Binary" in dataframe.columns:
            if dataframe["Is_Binary"].any():

                synthpop_nonmag_bin_cols = ['system_idx', 'zams_mass', 'Teff', 'L',
                                            'logg', 'isWR', 'mass', 'phase', 'metallicity',
                                            'log_a', 'e', 'i', 'Omega', 'omega', 'zams_mass_prim']

                dataframe.rename(columns={'Is_Binary': 'isMultiple', 'combined_logP': 'logP'}, inplace=True)

                counts_1 = dataframe.groupby("primary_ID")["primary_ID"].transform('count')

                id_counts = dataframe['primary_ID'].value_counts()
                counts_2 = dataframe['ID'].map(id_counts)

                dataframe['N_companions'] = counts_2.add(counts_1, fill_value=0)
                dataframe['N_companions'] = dataframe['N_companions'].fillna(0)

                dataframe["temp_id"] = dataframe["ID"]
                dataframe["temp_primary"] = dataframe["primary_ID"]

                temp_df = dataframe[['temp_id', 'obj_id']].copy()
                temp_df.rename(columns={'temp_id': 'temp_primary'}, inplace=True)
                merged_stars = dataframe.merge(temp_df, on='temp_primary', how='left', suffixes=('', '_new'))
                dataframe['system_idx'] = merged_stars['obj_id_new']

                temp_df = dataframe[['temp_id', 'zams_mass']].copy()
                temp_df.rename(columns={'temp_id': 'temp_primary'}, inplace=True)
                merged_stars = dataframe.merge(temp_df, on='temp_primary', how='left', suffixes=('', '_new'))
                dataframe['zams_mass_prim'] = merged_stars['zams_mass_new']

                dataframe.drop(columns=['temp_id', "temp_primary"], inplace=True)

                binary_dict = dataframe[~dataframe["isMultiple"].isin([0, 1])]
                binary_dict = binary_dict.reset_index(drop=True)
                dataframe = dataframe[dataframe["isMultiple"].isin([0, 1])]
                dataframe = dataframe.reset_index(drop=True)

                binary_dict.rename(columns={"feh": "metallicity", "grav": "logg", "rem_id": "phase"}, inplace=True)
                binary_dict["isWR"] = np.zeros(binary_dict.shape[0], dtype=int)
                combined_mass = binary_dict["zams_mass"] + binary_dict["zams_mass_prim"]
                semimajor_axis = np.cbrt((10 ** (binary_dict["logP"])) ** 2 * combined_mass)
                binary_dict.drop(columns=['logP'], inplace=True)

                binary_dict["log_a"] = np.log10(semimajor_axis)
                binary_dict["e"] = np.zeros(binary_dict.shape[0], dtype=int)
                x = np.random.uniform(0, 1, size=binary_dict.shape[0])
                y = np.random.uniform(0, 1, size=binary_dict.shape[0])
                z = np.random.uniform(0, 1, size=binary_dict.shape[0])
                binary_dict["i"] = np.arccos(x) * 180 / np.pi
                binary_dict["Omega"] = 360 * y
                binary_dict["omega"] = 360 * z

                synthpop_bin_mag_cols = []

                filters_ukirt = {"H", "J", "K"}
                filters_ubv = {"U", "B", "V", "R", "I"}
                for filter in filters_ukirt:
                    if f"ubv_{filter}" in binary_dict.columns:
                        binary_dict.rename(columns={f"ubv_{filter}": f"m_ukirt_{filter}"}, inplace=True)
                    else:
                        binary_dict[f"m_ukirt_{filter}"] = np.nan
                    synthpop_bin_mag_cols.append(f"m_ukirt_{filter}")

                for filter in filters_ubv:
                    if f"ubv_{filter}" in binary_dict.columns:
                        binary_dict.rename(columns={f"ubv_{filter}": f"m_ubv_{filter}"}, inplace=True)
                    else:
                        binary_dict[f"m_ubv_{filter}"] = np.nan
                    synthpop_bin_mag_cols.append(f"m_ubv_{filter}")

                cols_to_cut = []
                for col in binary_dict.keys():
                    if col not in (synthpop_nonmag_bin_cols + synthpop_bin_mag_cols + popsycle_nonmag_cols):
                        cols_to_cut.append(col)
                binary_dict.drop(columns=cols_to_cut, inplace=True)

                binary_dict = binary_dict.reset_index(drop=True)
                dataframe = dataframe.reset_index(drop=True)

                h5file = h5py.File(f"{self.output_root}_companions.h5", 'w')
                h5file['lat_bin_edges'] = lat_bin_edges
                h5file['long_bin_edges'] = long_bin_edges
                h5file.close()

                _bin_lb_hdf5(lat_bin_edges, long_bin_edges, binary_dict, f"{self.output_root}_companions")
                print(f"PopSyCLE formatted companions output saved in {self.output_root}_companions.h5")

                self.logger.info("Binary file generated.")
        ...
        for col in dataframe.keys():
            if col not in (synthpop_nonmag_cols + self.synthpop_mag_cols +
                           [self.model.populations[
                                0].extinction.A_or_E_type] + popsycle_nonmag_cols + self.mag_cols):
                cols_to_cut.append(col)
        ...
        for col in dataframe.keys():
            if col not in (popsycle_nonmag_cols + self.mag_cols):
                cols_to_cut.append(col)
        new_data = dataframe.drop(columns=cols_to_cut)
        ...
        _bin_lb_hdf5(lat_bin_edges, long_bin_edges, dataframe, self.output_root)
