...
pc_isochrones = None
pc_non_mag_cols = ['Zini', 'MH', 'logAge', 'Mini', 'int_IMF', 'Mass', 'logL', 'logTe', 'logg', 'label', 'McoreTP', 
                    'C_O', 'period0', 'period1', 'period2', 'period3', 'period4', 'pmode', 'Mloss', 'tau1m', 
                    'X', 'Y', 'Xc', 'Xn', 'Xo', 'Cexcess', 'Z', 'mbolmag']
pc_req_cols_to_mist = {'MH':'[Fe/H]_initial', 'logAge':'log10_isochrone_age_yr', 'Mini':'initial_mass', 'label':'phase'}
...
class ParsecColibri(EvolutionIsochrones, CharonInterpolator):
    ...
    def __init__(self, columns, use_global=True, **kwargs):
        ...
        # Get metallicities
        feh_pos = sorted([x[x.find('_mh_') + 4:x.find('_mh_') + 9]
            for x in glob.glob(
                f'{self.FOLDER}/{msys}/PARSECv1.2_COLIBRI*mh_p*.dat')])

        feh_neg = sorted([x[x.find('_mh_') + 4:x.find('_mh_') + 9]
            for x in glob.glob(
                f'{self.FOLDER}/{msys}/PARSECv1.2_COLIBRI_*mh_m*.dat')],
            reverse=True)
        ...
        if use_global:
            ...
            if pc_isochrones is not None:
                if np.array_equal(np.sort(np.unique(self.bands+self.none_mag_cols)).astype(str), np.sort(np.unique(pc_isochrones.keys())).astype(str)):
    ...
    def load_isochrones(self, magsys):
        ...
        for i, (magsys_name, chosen_columns) in enumerate(magsys.items()):
            ...
            for met_string, file_met in zip(self.met_to_file_iso, self.file_met):
                ...
                df.rename(columns=pc_req_cols_to_mist, inplace=True)
